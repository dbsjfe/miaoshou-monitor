...
推送链（2026-09-11 起**只有邮件一条**）：

```
邮件（唯一通道，无限量，QQ邮箱，账号见 config.json）✅ 已打通并实测送达
        ↓ 发送失败（SMTP 异常 / 授权码失效 / 网络不通）
    留在 pending 队列（带完整快照），下一轮自动重试，进程重启也不丢
```

**当前生效（2026-09-11 用户指定）**：`channel_order = ["email"]`，本地与云端一致。
Server酱 / WxPusher / 企业微信（应用消息 + 群机器人）/ PushPlus **已从代码里彻底删除**，
不是只从 `channel_order` 移除 —— `push_channels.py` 现在只剩 `EmailChannel`。
要重新启用某通道，得把类写回来并在 `Notifier` 里注册（删掉的原因见下文各节记录）。
`config.json` 里对应的字段也已清空，`set_gh_secrets.py` 与 workflow 的 env 映射同步收敛为 7 个 Secret。
...
> ## 🗄 以下「通道选型」章节已归档（2026-09-11）
>
> 用户最终决定**只保留邮件 + 电脑语音提醒**，下列通道的**代码已从 `push_channels.py` 删除**、
> 配置字段与 Secrets 映射也已移除。所以下面写的「代码保留、要用时填 key 加回顺序」**已不成立**，
> 要重新启用得把对应的类写回来。这些记录仍然留着，是为了说明**为什么不用它们**，
> 避免以后又花一轮时间去试。
...
> - **WxPusher** 无 IP 白名单、无实名、无费用，微信扫码 2 分钟配好 → 曾作备用通道
> - **Server酱** 免费版每天只有 5 条 → 曾作兜底，单量一上来必漏
> - **最终结论**：邮件一条路（无条数限制、免费、可长期留存）＋ 本机语音提醒
...
结论：**放弃应用消息**（代码已于 2026-09-11 从 `push_channels.py` 删除）。

另外还评估过群机器人通道（无 IP 限制）：
...
| **`push_channels.py`** | 通道层：**只有 SMTP 邮件**（`EmailChannel`）+ 限流 + 日额度计数（接口仍是多通道形状，方便将来扩展） |
...
| 本地推送通道 | ✅ 仅 email（唯一通道） | `test_push.py` 自检 |
...
| Actions Secrets | ✅ 已写入（2026-09-11 收敛为 7 个） | `set_gh_secrets.py` → 成功 / 失败 0 |

### 关键状态

- ✅ **生效实例**：`local/monitor.py`（v2）常驻运行，pidfile 单例 + 孤儿清理 + 自愈循环
  （kill -9 后 10 秒自动重启，实测有效）
- ✅ **推送通道（2026-09-11 用户指定）**：`channel_order = ["email"]`，**只有邮件一条**
...
- 🗑 **已删除的通道（2026-09-11）**：Server酱、WxPusher、企业微信应用消息（曾被 60020 可信IP 拦截）、
  群机器人、PushPlus。代码、配置字段、Secrets 映射、文档、安装包全部同步移除；
  下面各节的"选型踩坑"记录保留，用来解释**为什么不再用它们**
