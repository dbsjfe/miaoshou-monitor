...
| 路径 | 生效范围 | 说明 |
|---|---|---|
| 📧 **邮件** | 本机 + 云端 | 唯一推送通道。订单详情发到邮箱，手机邮件 App 直接弹窗 |
| 🔊 **电脑语音提醒** | 仅本机 macOS | 推送成功的同一秒念一句 + 弹通知横幅 |

> 曾经试过 Server酱（免费版每天 5 条）、WxPusher、企业微信（要可信 IP）、PushPlus（要认证费），
> 2026-09-11 全部移除 —— 邮件一条路就够，可靠性交给「失败重试队列 + 双执行体去重」而不是堆通道。
> 要加通道的话，`local/push_channels.py` 里写个带 `send(title, content)` 的类注册进 `Notifier` 即可。
...
邮件是**远程**提醒 —— 人在电脑前、手机不在手边时，来单可能要十几分钟后才看到。
...
> 2026-09-11 起只有邮件一个通道，Secrets 收敛为 **7 个**（妙手 2 + SMTP 5）。
> 之前写过的 `SERVERCHAN_SEND_KEY` / `WXPUSHER_*` / `WECOM_*` 已不再被 workflow 读取，
> 可以直接在仓库里删掉。
...
1. 先去平台**重置密钥**（妙手后台重置 AppSecret、邮箱里重新生成 SMTP 授权码）——这一步最关键
